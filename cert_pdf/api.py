...
def add_log(str):
    ...
    print('[%s] %s' % (datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), str), file = file)
    file.close()
...
def go_task(f, msg_str, print_suc = True):
    try:
        ...
        add_log('The job <%s> failed. Please see log file <%s> for details.' % (TOKEN, path_helpers.get_stage_log_dir(TOKEN)))
...
def issue_batch(import_path, export_path, pubkey, psw_file, itsc = None, name_pattern = '|DOCID|-|NAME|'):
    ...
    if file_count == 0:
        ...
        add_log('The job <%s> failed. Because there is no valid PDF file found.' % TOKEN)
